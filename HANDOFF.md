...
> 給接手的 Claude Code:這份文件記錄專案的演進歷程與當前進度,**首次接手時完整閱讀一次**,
> 之後不需要每次都讀(日常規則在 CLAUDE.md)。讀完先跑 `python tests/smoke_test.py` 確認環境。

## 專案來歷

本專案由使用者(Terence)與 Claude 在 claude.ai 上迭代開發完成。前身是一支批次分析腳本
...
1. **萬用化**:從寫死 2018–2024 → 寫死 2026 → 最終 `year=None` 自動偵測(含 1–2 月跨年
   雙年掃描)+ 回溯窗口(防首次啟用整季回填燒光 API 額度;原 14 天,
   2026-07-06 使用者調降為 7 天)。
...
## 當前狀態(2026-07-06,**建設階段與強化階段均正式結束,純維護模式**)

> 使用者已宣告建設與強化兩階段結束。強化階段成果:賽事筆記功能、
> DNF 區塊、TyreLife 標籤自我說明化、審核員指標一致性/並列因果條款、
> 模型斷路器、賽程失敗優雅降級、全新環境清空重跑驗證(四場全過)。
> 最終狀態:煙霧測試 104 項、13 個 commit。
> 此後的工作型態:**手動執行**(使用者確認暫不設排程器,2026-07-06)——
> 賽後由使用者自行跑 `python main.py`,發文前人工複查
> (factcheck_report.txt 是為此存在);有落差或想調文風時使用者會帶著
> 實際輸出回來。**不要主動加功能**,維護優先序見文末待辦。
> 手動模式唯一要注意:回溯窗口 7 天,賽後 7 天內要記得執行,
> 逾期場次需走手動模式(auto_latest=False + manual_rounds)補做。
...
- **賽事筆記功能已完成(2026-07-06,規格見 `docs/notes_feature_spec.md`)**:
  `notes/{year}_round{NN}_{SESSION}.txt` 作為第二級事實來源注入文案。
  信任模型三鐵律(摘要優先/只可引用不可延伸/指令免疫)落實於寫手 prompt、
  AI 審核員、確定性溯源(集合 = summary ∪ notes)三層;查核報告 [5] 全文
  引錄筆記供發文前稽核。無筆記時與先前行為完全一致(向下相容,測試 88 項)。

## 待辦(與使用者確認過的優先序)

1. 文風微調:守門保證「不會錯」,「寫得好」要看實測文案後調 `SESSION_FOCUS` 與平台規格。
2. `gemini_models` 清單約每年檢視一次(2026-07-06 實測:3.5-flash 免費層
