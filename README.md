...
賽後數據自動分析，產出 FB/IG 社群文案。每個數字都必須能溯源回官方計時資料，不合格就不輸出。

每場排位賽 / 衝刺排位 / 衝刺賽 / 正賽結束後執行，從 FastF1 取得官方計時資料，自動產出：分析數據（CSV/Excel）、文字摘要、發文配圖（PNG），以及經過三重守門與事實查核的 FB + IG 雙版本文案。使用者只需做兩件事：複查事實查核報告、複製文案發文。

把數據丟給 LLM 寫文案很容易，難的是確保它沒有在數字上說謊。一篇讀起來順暢的貼文，秒差、名次、停站次數只要錯一個，對數字敏感的 F1 社群就會抓到，而寫錯的貼文比不發文傷害更大。因此整套系統的立場是**寧缺勿錯**：寧可這場不產出，也不產出一篇可能有錯的文案。

三個機制支撐這個立場：

- **唯一事實來源**：所有事實一律從 FastF1 原始圈資料推導後寫進 `summary.txt`，並交叉驗證回原始資料；文案只能引用 summary，不得自行取材。
- **三重守門**：靜態規則（用語與格式）→ 確定性數字溯源（不靠 AI，逐一比對每個秒數、名次、停站數、圈號）→ LLM 審核員（比對摘要逐項查核）。任一關不過就帶著退稿意見重寫，重寫上限用盡則拒絕輸出。
- **冪等契約**：`social_post.txt` 存在即代表該場完成，任何失敗路徑都不會留下它。因此可放進排程器反覆執行，失敗的場次下次會自動重試，已完成的場次不會重複消耗 API 額度。

技術組成：Python、FastF1、pandas、matplotlib、Google Gemini API；另有選用的 Flask 本機介面（`app.py`）可瀏覽產出、一鍵複製文案。三支測試（單元 / 端到端 / 網頁介面）全部離線執行，不需網路與 API key。
...
預設為自動模式：自動偵測目前賽季、找出「最近完賽但未處理」的場次。建議掛上排程器於比賽日每 2 小時執行（冪等設計，重複執行不浪費 API 額度）。
...
資料只往一個方向流：**原始資料 → summary.txt → 文案**。文案裡的每個數字都必須能溯源回 summary（或賽事筆記），這條線是整個系統的核心約束。
...
summary 只有數據，沒有事故/天氣/罰時等敘事。想讓文案提到這些，在專案根目錄建 `notes/{年份}_round{站次兩位數}_{場次代碼}.txt`，例如英國站正賽：`notes/2026_round09_R.txt`（場次代碼：Q / R / SQ / S）。
...
**寫作指引：一行一個事實**。避免用「並」「因此」或逗號在同一行串接多件事；想表達因果就明確寫出（「A 導致 B」），沒寫因果的並列事實，AI 會被禁止自行連成因果。
...
**改筆記後想重生文案**：刪除該場的 `social_post.txt` 再跑一次，或以 `force_rerun=True` 執行（已產出的場次預設會跳過，這是冪等設計）。
